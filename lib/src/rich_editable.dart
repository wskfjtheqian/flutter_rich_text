...
import 'package:rich_text_edit/src/rich_span.dart';
...
typedef RichSelectionChangedCallback = void Function(TextSelection selection, RichSelectionChangedCause? cause);

/// Signature for the callback that reports the app private command results.
typedef RichAppPrivateCommandCallback = void Function(String, Map<String, dynamic>);
...
final Map<LogicalKeySet, Intent> richScrollShortcutOverrides = <LogicalKeySet, Intent>{
...
///  * Learn how to use a [RichTextEditingController] in one of our [cookbook recipes](https://flutter.dev/docs/cookbook/forms/text-field-changes#2-use-a-texteditingcontroller).
class RichTextEditingController extends ValueNotifier<TextEditingValue> {
  ...
  /// string.
  RichTextEditingController({String? text, this.textToRichSpan}) : super(text == null ? TextEditingValue.empty : TextEditingValue(text: text));

  /// Creates a controller for an editable text field from an initial [TextEditingValue].
  ///
  /// This constructor treats a null [value] argument as if it were
  /// [TextEditingValue.empty].
  RichTextEditingController.fromValue(TextEditingValue? value, {this.textToRichSpan})
      : assert(
...
/// and select all.
///
/// [RichEditableText] and its derived widgets have their own default [RichToolbarOptions].
/// Create a custom [RichToolbarOptions] if you want explicit control over the toolbar
/// option.
class RichToolbarOptions {
  ...
  ///
  /// If [RichEditableText.readOnly] is set to true, cut will be disabled regardless.
  ///
  /// Defaults to false. Must not be null.
  final bool cut;
  ...
  ///
  /// If [RichEditableText.readOnly] is set to true, paste will be disabled regardless.
  ///
  /// Defaults to false. Must not be null.
  final bool paste;
...
class RichEditableText extends StatefulWidget {
  ...
  RichEditableText({
    ...
    this.toolbarOptions = const RichToolbarOptions(
      copy: true,
      cut: true,
      paste: true,
      selectAll: true,
  ...
  final RichToolbarOptions toolbarOptions;
  ...
  /// {@endtemplate}
  final RichAppPrivateCommandCallback? onAppPrivateCommand;

  /// {@template flutter.widgets.editableText.onSelectionChanged}
  /// Called when the user changes the selection of text (including the cursor
  /// location).
  /// {@endtemplate}
  final RichSelectionChangedCallback? onSelectionChanged;
  ...
  ///
  /// The [mouseCursor] is the only property of [RichEditableText] that controls the
  ...
  /// pointer events, see [RichRenderEditable] and [RichRenderEditable.ignorePointer].
  ///
  ...
  ///
  /// If this property is null, [RichRenderEditable.preferredLineHeight] will be used.
  /// {@endtemplate}
  ...
  ///{@macro flutter.rendering.RichRenderEditable.cursorOffset}
  final Offset? cursorOffset;

  ///{@macro flutter.rendering.RichRenderEditable.paintCursorAboveText}
  ...
  /// [RichRenderEditable.selectionEnabled].
  /// {@endtemplate}
  ...
  /// [RichEditableText].
  ///
  /// If a restoration id is provided, the [RichEditableText] will persist its
  /// current scroll offset and restore it during state restoration.
  ///
  /// The scroll offset is persisted in a [RestorationBucket] claimed from
  /// the surrounding [RestorationScope] using the provided restoration ID.
  ///
  /// Persisting and restoring the content of the [RichEditableText] is the
...
class RichEditableTextState extends State<RichEditableText>
  ...
  final RichClipboardStatusNotifier? _clipboardStatus = kIsWeb ? null : RichClipboardStatusNotifier();
  ...
  /// update via [updateEditingValue], or when [RichEditableText] calls
  ...
  void updateEditingValue(TextEditingValue value) {
    ...
    if (value.text == _value.text && value.composing == _value.composing) {
      ...
      _handleSelectionChanged(value.selection, renderEditable, RichSelectionChangedCause.keyboard);
    } else {
      hideToolbar();
  ...
  void _onFloatingCursorResetTick() {
    ...
    if (_floatingCursorResetController.isCompleted) {
      ...
      if (_lastTextPosition!.offset != renderEditable.selection!.baseOffset)
        ...
        _handleSelectionChanged(TextSelection.collapsed(offset: _lastTextPosition!.offset), renderEditable, RichSelectionChangedCause.forcePress);
  ...
  ///
  /// Throws an error in debug mode if this [RichEditableText] is not in a batch
  /// edit.
  void endBatchEdit() {
    _batchEditDepth -= 1;
    assert(
  ...
  RevealedOffset _getOffsetToRevealCaret(Rect rect) {
    if (!_scrollController!.position.allowImplicitScrolling) return RevealedOffset(offset: _scrollController!.offset, rect: rect);
  ...
  void _handleSelectionChanged(TextSelection selection, RichRenderEditable renderObject, RichSelectionChangedCause? cause) {
    ...
    SchedulerBinding.instance!.addPostFrameCallback((Duration _) {
      ...
      if (_selectionOverlay?.selectionControls != null) {
        ...
        final Offset anchor = _selectionOverlay!.selectionControls!.getHandleAnchor(
          RichTextSelectionHandleType.collapsed,
          lineHeight,
        );
  ...
  void _formatAndSetValue(TextEditingValue value) {
    ...
    final bool textChanged = _value.text != value.text || (!_value.composing.isCollapsed && value.composing.isCollapsed);
    ...
    if (textChanged) {
      ...
      // upcoming PR.
      if (widget.inputFormatters?.isNotEmpty ?? false) value = _whitespaceFormatter.formatEditUpdate(_value, value);
    }
  ...
  /// when [RichRenderEditable.ignorePointer] is true.
  ...
  TextSpan buildTextSpan() {
    if (widget.obscureText) {
      ...
      if ((defaultTargetPlatform == TargetPlatform.android || defaultTargetPlatform == TargetPlatform.iOS || defaultTargetPlatform == TargetPlatform.fuchsia) &&
          !kIsWeb) {
